...
function markupLink($link='', $name='', $tag='', $style='link', $target='self', $group='', $format='relative', $prefix='nope'){
	...
	$prepare['link'] = '';	
	if($format == 'external'){$prepare['link'] .= $link;}
	else {
...
function responsiveImg($image=''){
	if(isEmpty($image)){return FALSE;}

	$image = pathTo($image, $path);

	#prepare and return
	if(isIE('==', 6)){$image .= '-ie6';}
	if(defined('REWRITE_URL') && REWRITE_URL == 'sure'){return $image .='.png';}
	else {
		if(is_file($image.'.png')){return $image .='.png';}
...
function slideImg($name='1', $caption='', $alt=''){
	...
	$slideapend = strtolower(Device::is());

	#prepare image file
	$image = PATH_SLIDE.PS.$name.'-'.$slideapend.'.jpg';
	if(!is_file($image)){$image = PATH_SLIDE.PS.$name.'.jpg';}
	if(!is_file($image)){$image = PATH_ASSET.PS.'slide-'.$name.'-'.$slideapend.'.jpg';}
	if(!is_file($image)){$image = PATH_ASSET.PS.'slide-'.$name.'.jpg';}
...
function layout($layout='current'){
	if($layout == 'current' || isEmpty($model)){$layout = erko3::page('name');}
	...
	if(isEmpty($value)){
		...
		if(!empty($heading) && array_key_exists($page, $heading)){
			...
			if($page == 'index'){
				...
				if(!empty(erko3::$brand) && !empty(erko3::$name)){$pageHeading .= ' | ';}
				if(!empty(erko3::$brand)){$pageHeading .= capitalize(erko3::$brand);}
			}
			else {
...
function responsiveDate(){
	...
	elseif($device == 'tablet'){$date = do_date('dateD3');}
	else {$date = doDate();}
