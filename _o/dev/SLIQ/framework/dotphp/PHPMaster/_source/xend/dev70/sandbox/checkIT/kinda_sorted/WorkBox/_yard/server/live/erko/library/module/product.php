...
$filta = ' ';
if(!empty($_GET['cat'])) {$filta = ' WHERE category = '.$_GET['cat']. '  ';}
mysql_select_db($database_dbcon, $dbcon);
$query_dataset = "SELECT * FROM product".$filta."ORDER BY product ASC";
...
<table border="0" cellspacing="0" cellpadding="0" class="viewTabHr" width="100%">
 ...
 <tr class="tabTitle">
    ...
    <th align="left" valign="middle" scope="col">product</th>
  ...
  <tr>
      ...
      <td><?php echo $row_dataset['product']; ?></td>
      <td><?php echo $row_dataset['acronym']; ?></td>
      <td>
	  	<?php 
			if($row_dataset['category']  !=='none') {
			$query = "SELECT category, buid FROM category WHERE buid = ".$row_dataset['category'] ;
			$data = mysql_query($query, $dbcon) ;#or print_r(mysql_error());
			$row_data = mysql_fetch_assoc($data);
			echo '<a href="./?link=product&cat='. $row_dataset['category'].'" title="Only from '.$row_data['category'].'">'.$row_data['category'].'<a/>';
			}  else { echo '-';}
		?>
        </td>
      <td align="center"><a href="./?link=modify_product&prod=<?php echo $row_dataset['suid']; ?>">modify </a>| <a href="./?link=delete_product&prod=<?php echo $row_dataset['suid']; ?>">delete</a></td>
