...
if(!getRecord('isRecord',$BindID,'question')){
  ...
  $query_rsSameRecord = "SELECT * FROM exam_question WHERE `BindID` = '$BindID'";
  ...
  if(!empty($_POST['DeleteBtn']) && $_POST['ConfirmDelete'] == 'DeleteIT'){
 	 //DELETE THE RECORD
      $deleteSQL = "DELETE FROM `exam_question` WHERE `BindID` = '$BindID'";
 ...
 <?php echo $isNotify;?>
  ...
  <table border="0" cellspacing="0" cellpadding="0"  class="hrView">
    <tr>
      ...
      <td colspan="2"><p>Are you sure you want to delete<b><?php if(!empty($row_rsSameRecord['Question'])){echo ' '.$row_rsSameRecord['Question'];}?></b>?</td>
    </tr>
    <tr>
      <td colspan="2">&nbsp;</td>
    </tr>
