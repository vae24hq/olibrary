...
function self_link($rewrite='', $domain=''){
	if(is_empty($rewrite) && defined('REWRITE_URL') && REWRITE_URL){$rewrite = REWRITE_URL;}
	$link = grab_link('uri'); $chore = '';
	if($link != 'default'){$chore .= do_link($link, grab_link('type'), $rewrite);}
	else {$chore .= do_link($link, 'page', $rewrite);}
	if(!is_empty($domain)){$chore = $domain.$chore;} elseif($link != 'default'){$chore ='.'.$chore;}
	return $chore;
}
