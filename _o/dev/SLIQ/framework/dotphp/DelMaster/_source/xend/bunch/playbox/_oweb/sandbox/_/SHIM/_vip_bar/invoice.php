...
$query_DataSet = sprintf("SELECT * FROM pos_vipbar_sales WHERE InvoiceNo = %s ORDER BY PosItemName ASC", GetSQLValueString($colname_DataSet, "text"));
...
if ((isset($_POST["run_process"])) && ($_POST["run_process"] == "go")) {
	...
	$insertSQL = sprintf("INSERT INTO `pos_vipbar_sales` (suid, tuid, fuid, InvoiceNo, PosItemName, Qty, UnitPrice, Amount) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
...
$insertGoTo = "./?url=invoice_vip-bar&invoice={$_SESSION['InvoiceNo']}&loc=$loc";
...
if(isset($_POST['DoReciept'])) {
	mysql_select_db($database_dbcon, $dbcon);
	$updatesql = "UPDATE `pos_vipbar_sales` SET RecordType = 'reciept', ItemStatus = 'sold' WHERE InvoiceNo = '{$_SESSION['InvoiceNo']}'";			
...
$updateGoTo = "./?url=notification&loc=$loc&act=vipbar-reciept-prepared&success";
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="inputTab"> 
           <tr>
            ...
            <td class="buttonCell">
              ...
              <a href="?url=items_vip-bar&act=pos&loc=<?php echo $loc; ?>" class="btnUpdate">ADD MORE ITEMS TO THIS ORDER</a>
              &nbsp;&nbsp;or&nbsp;
