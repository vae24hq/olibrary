...
$query_DataSet = sprintf("SELECT * FROM pos_bushbar_sales WHERE InvoiceNo = %s ORDER BY PosItemName ASC", GetSQLValueString($colname_DataSet, "text"));
...
if ((isset($_POST["run_process"])) && ($_POST["run_process"] == "go")) {
	...
	$insertSQL = sprintf("INSERT INTO `pos_bushbar_sales` (suid, tuid, fuid, InvoiceNo, PosItemName, Qty, UnitPrice, Amount) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
  ...
  $Result1 = mysql_query($insertSQL, $dbcon) or die(mysql_error());
  			...
  			$newq = ($_POST['QtyInStore'] - $_POST['Qty']);
			$upsql = "UPDATE inventory_item SET QtyInStore = '$newq' WHERE suid='{$_POST['ItemSuid']}'";			
			$runq = mysql_query($upsql, $dbcon) or die(mysql_error());

$insertGoTo = "./?url=invoice_bush-bar&invoice={$_SESSION['InvoiceNo']}&loc=$loc";
...
if(isset($_POST['DoReciept'])) {
	mysql_select_db($database_dbcon, $dbcon);
	$updatesql = "UPDATE `pos_bushbar_sales` SET RecordType = 'reciept', ItemStatus = 'sold' WHERE InvoiceNo = '{$_SESSION['InvoiceNo']}'";			
...
$updateGoTo = "./?url=notification&loc=$loc&act=bushbar-reciept-prepared&success";
...
<link rel="stylesheet" type="text/css" href="../script/css/screen.css"/>
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="viewTabHr">
    ...
    <tr class="cellTitle">
      ...
      <th width="40" align="center" valign="middle" scope="col">QTY</th>
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="inputTab"> 
           <tr>
            ...
            <td class="buttonCell">
              ...
              <a href="?url=items_bush-bar&act=pos&loc=<?php echo $loc; ?>" class="btnUpdate">ADD MORE ITEMS TO THIS ORDER</a>
              &nbsp;&nbsp;or&nbsp;
