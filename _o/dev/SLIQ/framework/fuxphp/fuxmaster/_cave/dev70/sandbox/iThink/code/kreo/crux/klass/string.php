...
class string {
	...
	public static function spaceToChar($string, $value, $inverse='nope'){
		if($inverse == 'yeah'){
	...
	public static function trim($string, $trim='odefault', $char=''){
		if(!empty($string)){
			...
			if($trim == 'ospace'){$string = preg_replace('/\s+/', '', $string);}

			#Remove (space|character|sting), from edges
			if($trim == 'ochar'){$string = trim($string, $char);}
		}
		return $string;
	}
}?>
