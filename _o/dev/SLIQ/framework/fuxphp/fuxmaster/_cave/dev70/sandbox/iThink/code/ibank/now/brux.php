...
function runQuery($query, $record='fetchAll'){
	global $database; global $connect; $result = false;
	$data = mysql_query($query, $connect) or die(mysql_error());

	if($record == 'totalRows'){$result = mysql_num_rows($data);}
	elseif($record == 'affectedRows'){$result = mysql_affected_rows();}
	elseif($record == 'fetchAll' || $record == 'fetchRow'){
		$rows = array();
		while ($row = mysql_fetch_assoc($data)){$rows[] = $row;}
		$result['rows'] = $rows;
		$result['totalRows'] = mysql_num_rows($data);
	}
	elseif(!empty($record)){
...
function calcTransBal($user=''){
	...
	$dataCR = mysql_query($qryCR, $connect) or die(mysql_error());
	$dataDR = mysql_query($qryDR, $connect) or die(mysql_error());
	$rowdataCR = mysql_fetch_assoc($dataCR);
	$rowdataDR = mysql_fetch_assoc($dataDR);
