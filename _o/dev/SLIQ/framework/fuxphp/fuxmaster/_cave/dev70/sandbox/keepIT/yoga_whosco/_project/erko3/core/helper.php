...
function render($device='desktop'){
	...
	return '<a href="'.$link.'" title="switch to '.$device.'" class="renderlink">'.ucfirst($device).'</a>';
}
...
function staleBrowser($browser=''){
	...
	$stale .= '	<p>You may reach us at <a href="mailto:'.erko::$webadmin.'" target="_blank" class="email">'.erko::$webadmin.'</a></p>'."\n";
	$stale .= '	</div>'."\n";
...
function notFound($data='', $return='display'){ #display | email | sms | log | all
	...
	$notice .= '<a href="mailto:'.erko::$webadmin.'" target="_blank" class="paralink">'.erko::$webadmin.'</a></p>'."\n".'		</div>'."\n";
...
function heading($value=''){
	if(isEmpty($value)){
		...
		if(!empty($heading) && array_key_exists($page, $heading)){
			...
			if($page == 'index'){
				if(!empty(erko::$name)){$pageHeading .= erko::$name;}
