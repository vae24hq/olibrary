 ...
 * ================================================================================================================================
 * PHP | helper::core ~ helper functions | Dependency » +
 */
...
function pathTo($dir='', $format='relative'){#[relative|absolute]
	...
	if($format == 'absolute' && !isEmpty(erko::site())){
		$path = erko::site().'/'.$path;
	}
	elseif($format == 'relative'){
		if(REWRITE_URL == 'sure' && erko::page() != 'index'){
			if(inString(erko::page('url'), '_')){
				#check how many '_' you have in page name {'_' indicates '/' in url}
				$pos = posChar(erko::page('url'), '_', 'all');
				$count = count($pos);
				$slash = '';
...
function markupLink($link='', $name='', $tag='', $style='link', $target='self', $group='', $format='relative', $prefix='nope'){
	...
	$prepare['name'] = trim(wordAsCap($name));
	...
	return '<a href="'.$prepare['link'].'" title="'.trim($prepare['tag']).'"'.$prepare['style'].$prepare['target'].'>'.$prepare['name'].'</a>';
}
...
function navLink($link='', $name='', $tag='', $prefix='nope', $format='relative', $style='navlink'){
	return markupLink($link, $name, $tag, $style, 'self', '', $format, $prefix);
}


// Make group navigation link
function navLinkGroup($group, $link='', $name='', $activate ='', $tag='', $style='navlink'){
...
function render($device='desktop'){
	...
	return '<a href="'.$link.'" title="render '.$device.'" class="render">'.ucfirst($device).'</a>';
}
...
function resImg($image='', $path='image'){
	...
	#prepare and return
	if(isIE('==', 6)){$apend = 'ie6';}
	elseif(isOperamini()){$apend = 'omini';}
	else {$apend = Device::is();}
...
function staleBrowser($browser=''){
	...
	$stale .= '	<p>Opps, we are sorry for this awkwardness <br>You appear to be using a ';
	if($browser == 'ie'){$stale .= 'very stale version of Internet Explorer ';}
	else {$stale .= 'browser or a very stale version of a browser. ';}
	$stale .= 'that we do not support. <br>Please upgrade to a more recent version or download other modern browsers. <br>'."Don't know where to start? - Try ".makeLinkExternal('http://www.google.com', 'Google™', '', 'Search with Google.com').'</p>'."\n";
	$stale .= '	<p>You may reach us at <a href="mailto:'.erko::$admin.'" target="_blank" class="email">'.erko::$admin.'</a></p>'."\n";
	$stale .= '	</div>'."\n";
...
function notFound($data='', $return='display'){ #display | email | sms | log | all
	...
	$notice .= '	<h2 class="heading">'.capitalize(cleanup($data['name'])).' - Not Found!</h2>'."\n";
	...
	$notice .= 'Please return to '.makeLink('index', erko::$name).'</p>'."\n";
	$notice .= '		<p>You may report this issue to us via ';
	$notice .= '<a href="mailto:'.erko::$admin.'" target="_blank" class="paralink">'.erko::$admin.'</a></p>'."\n".'		</div>'."\n";
	if(SET_MODE == 'development'){
		$notice .= '		<pre><b>Name:</b> '.$data['name']."\t".'<b>Type:</b> '.$data['type']."\t".'<b>File:</b> '.$data['file'].'</pre>'."\n";
	}
	$notice .= '	</div>'."\n".'	</article>'."\n";
...
function view($view='current'){
	if($view == 'current' || isEmpty($view)){$view = erko::page();}
	return erko::content($view, 'page');
}
...
function slice($slice='index', $type=''){
	...
	return erko::content($slice, 'slice');
}

// Loads default file
function launch(){
	return erko::content('index', 'slice');
}

// Loads current controller
function organizer($organizer='current'){
	if($organizer == 'current' || isEmpty($organizer)){$organizer = erko::page();}
	return slice($organizer, 'organizer');
}

// Loads current controller
function model($model='current'){
	if($model == 'current' || isEmpty($model)){$model = erko::page();}
	return slice($model, 'model');
}

// Loads specific layout
function layout($layout='current'){
	if($layout == 'current' || isEmpty($layout)){$layout = erko::page();}
	return slice($layout, 'layout');
}

// Loads specific piece
function piece($piece='current'){
	if($piece == 'current' || isEmpty($piece)){$piece = erko::page();}
	return slice($piece, 'piece');
}

// Loads specific outline
function outline($outline='current'){
	if($outline == 'current' || isEmpty($outline)){$outline = erko::page();}
	return slice($outline, 'outline');
}

// Loads specific code library
function code($code='current'){
	if($code == 'current' || isEmpty($code)){$code = erko::page();}
	return slice($code, 'code');
}

// Loads specific plugin library
function plug($plug='current'){
	if($plug == 'current' || isEmpty($plug)){$plug = erko::page();}
...
function heading($value=''){
	if(isEmpty($value)){
		...
		if(!empty($heading) && array_key_exists($page, $heading)){
			...
			if($page == 'index'){
				if(!empty(erko::$squat)){$pageHeading .= erko::$squat;}
				if(!empty(erko::$slogan)){$pageHeading .= ' - '.sentence(erko::$slogan);}
			}
			else {
...
function breadcrumb($crumb=''){
	#build crumb
	if(isEmpty($crumb)){
		$brand = erko::$name; $page = erko::page(); $device = Device::is();

		if($page == 'index'){$crumb .= 'Welcome to ';}

		#add brand name to breadcrumb
		if($device == 'phone' && !isEmpty(erko::$squat)){$brand = stringSwap(erko::$squat, 'imited', 'td');}
		elseif($device == 'tablet' && !isEmpty(erko::$name)){$brand = stringSwap(erko::$name, 'imited', 'td');}
		elseif($page == 'index' && !isEmpty(erko::$name)){$brand = erko::$name;}
		elseif(!isEmpty(erko::$brand)){$brand = erko::$brand;}

		$crumb .='<a href="'.pathTo('index').'" title="'.erko::$squat.'" class="brand">'.$brand.'</a>';
...
function copyright($from='', $reserved='nope', $to='now'){
	...
	$copyright ='';
	if(Device::is() != 'phone' && isEmpty($from)){$copyright .='Copyright';}
	$copyright .=' &copy;';

	#prepare link (open)
	$link = '<a href="'.pathTo('index').'" class="brand" title="'.erko::$squat.'">';

	#prepare brand
	$brand ='';
	if(!isEmpty($link)){$brand .= $link;}

	if(Device::is() == 'phone' && !isEmpty(erko::$brand)){$brand .=erko::$brand;}
	elseif(Device::is() == 'tablet'  && !isEmpty(erko::$brand)){
		$brand .= stringSwap(erko::$name, 'imited', 'td');
	}
	elseif(!isEmpty(erko::$name)){$brand .= erko::$name;}
