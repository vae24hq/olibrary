...
if ((isset($_POST["Form_Update"])) && ($_POST["Form_Update"] == "updateIT")){
	$updateSQL = sprintf("UPDATE `client` SET `uname`=%s, `pin`=%s, `email`=%s, `securityhint`=%s, `accountno`=%s, `acctype`=%s, `swift`=%s, `currency`=%s, `status`=%s, `accbal`=%s, `billing`=%s, `income`=%s, `outgo`=%s, `statement`=%s, `title`=%s, `fname`=%s, `mname`=%s, `lname`=%s, `phone`=%s, `birthdate`=%s, `sex`=%s, `city`=%s, `state`=%s, `country`=%s, `address`=%s, `nationality`=%s,  `idtype`=%s, `idnumber`=%s, `idexpiration`=%s, `assetworth`=%s, `creditworth`=%s, `asset`=%s, `service`=%s, `comment`=%s, `annual`=%s, `occupation`=%s, `calltime`=%s, `nok`=%s, `nokemail`=%s, `nokphone`=%s, `nokaddress`=%s, `nokrelationship`=%s WHERE `id`=%s",
	...
	<div class="page"><?php if(getClient('totalRows') == 0){?><p class="box bg-danger text-danger">No record found</p><?php } else {?>
		...
		<table class="table table-hover table-bordered table-striped">
			...
			<tr>
				...
				<td><input name="income" type="text" id="income" class="form-control" value="<?php echo getClient('income');?>"></td>
				<td><input name="outgo" type="text" id="outgo" class="form-control" value="<?php echo getClient('outgo');?>"></td>
				<td><textarea name="statement" id="statement" cols="45" rows="5" class="form-control"><?php echo getClient('statement');?></textarea></td>
			</tr>
			</tbody>
		</table>
