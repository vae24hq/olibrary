...
class erko {
	...
	public static function setting($data=''){
		if(!is_empty($data) && !is_array($data)){return is_msg('U-Erko/01|01');}
	...
	private static function meta_data($data='description'){
		...
		if(!empty($metaData) && array_key_exists($meta, $metaData)){
			...
			if($data=='description' && !empty(self::$slogan)){$task .= ' - '.ucfirst(self::$slogan);}
			if($data=='keywords' && !empty(self::$basetag)){$task .= ', '.ucwords(self::$basetag);}
		}
	...
	private static function auto_prep_object($type='page'){
		...
		elseif($type=='link'){
			if($content['module'] !='index'){$task .= $content['module'].'_';}
			$task .= $content['operation'];
		}
		elseif($type=='page'){
	...
	public static function charset(){
 	...
 	public static function xua_compatible(){
 		$chore ='';
 		if(browser_info()=='ie'){$chore ='<meta http-eqbaseuiv="X-UA-Compatible" content="IE=edge,chrome=1">'."\n";}
 		echo $chore;
 	}
	...
	public static function favicon($path=''){
		...
		if(browser_info()=='ie' || browser_info()=='operamobi' || browser_info()=='operamini'){
			$chore ='<link rel="icon" type="image/x-icon" href="'.$path.'favicon.ico">'."\n";
		} else {$chore ='<link rel="icon" type="image/png" href="'.$path.'favicon.png">'."\n";}
		if(device::is() !='desktop'){
			$chore .='<link rel="apple-touch-icon-precomposed" href="'.$path.'apple-touch-icon-precomposed.png">'."\n";
			$chore .='<link rel="apple-touch-icon" sizes="72x72" href="'.$path.'apple-touch-icon-72x72.png">'."\n";
			$chore .='<link rel="apple-touch-icon" sizes="114x114" href="'.$path.'apple-touch-icon-114x114.png">'."\n";
			$chore .='<link rel="apple-touch-icon" sizes="144x144" href="'.$path.'apple-touch-icon-144x144.png">'."\n";
		}
	...
	public static function css($flex='device', $ui='auto'){
		...
		elseif($device=='phone'){$device ='small';}
		...
		//prep for $chore
		$chore = '';
		$chore .='<link rel="stylesheet" type="text/css" href="'.path_to('css').'stablyzr.css'.$ver.'">'."\n";
		$chore .='<link rel="stylesheet" type="text/css" href="'.$ui.'base.css'.$ver.'">'."\n";

		//load css layout for device or browser
		if($flex =='device' || $flex =='layout'){
	...
	public static function js($path=''){
		...
		$chore .='<script type="text/javascript" src="'.$path.'responsive-nav.js"></script>'."\n";
