 ...
 * PHP | util:time - functions & classes for time
 * Dependency » 
 */
	...
	function doDate($format='cigna',$date='today'){
		#prep date
		if($date=='today'){$dateIs = time();}
		elseif(!$dateIs = strtotime($date)){$dateIs = $date;}
		
		#prep format
		if($format=='cigna'){$formatIs = 'l, F d, Y h:i:s A';}
	...
	function doTime($format='time',$time='now'){
		...
		#prep format
		if($format=='cigna'){return doDate('cigna',$time);}
	...
	function doTimeStamp($string){
		...
		else {die("can't convert [" .$string. '] to timestamp');}
	}
	...
	function timeBetween($past='',$future='now',$unitAs='long'){
		if($past>=$future){die("can't compute time difference");}
		$time = $future - $past;
		if($unitAs=='short'){
			$units = array(
			'y'=>$time / 31556926 % 12,
