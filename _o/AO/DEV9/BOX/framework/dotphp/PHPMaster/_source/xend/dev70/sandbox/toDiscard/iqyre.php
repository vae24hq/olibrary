...
class iQYRE {
	...
	public static function Breadcrumb($crumb=''){
		if(empty($crumb)){
			$crumb = '<a href="'.self::SiteURL().'" title="'.self::$Name.'" class="topbrand">'.self::$Brand.'</a>';
			if(self::Page() != 'index'){$crumb .= ' » '.ucwords(StringSwap('-',' ', self::Page()));}
		}
		return $crumb;
	}
}
