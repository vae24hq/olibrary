...
$query_DataSet = "SELECT * FROM inventory_item WHERE InvStore = '665232'";
		...
		if(isset($_GET['show']) && $_GET['show'] != '') {
			...
			elseif($showOnly == 'cigar') {
				$query_DataSet .= " AND InvCat = 'TM7359360929307HC1362932487SZ' AND ItemType = 'comodity'";
			}
			else {
       ...
       <tr class="tabContent"> 
        <td height="30" align="center" valign="middle" scope="col"><?php echo $rowNo = $rowNo + 1; ?></td>
          ...
          <?php $showPOS = showUrlArea("pos"); if($showPOS == 'yes') { ?>
           ...
           <form action="./?url=invoice_bush-bar&invoice=<?php echo $InvoiceNo?>&loc=<?php echo $loc;?>" method="post" name="posForm" id="posForm">
...
<input type="submit" name="POSBtn" id="POSBtn" value="Sell" class="btnSmall"/>
        ...
        <?php if($showManage == 'yes') { ?>
         <td align="center" valign="middle">
           <a href="?url=update-item_bush-bar&item=<?php echo $row_DataSet['suid']; ?>&loc=<?php echo $loc; ?>" class="btnUpdate">Go</a>
         </td> 
         <td align="center" valign="middle"> 
           <a href="?url=delete-item_bush-bar&item=<?php echo $row_DataSet['suid']; ?>&loc=<?php echo $loc; ?>" class="btnDelete">Go</a>
         </td>
         <?php } ?>
        </tr>
